...
import {Colors, ColorType} from 'values';
...
class TravelSettings extends Component<Props> {
  render() {
    ...
    if (outCall) {
      travelSettings.push(
        <TravelSetting badgeText="Mobile" badgeColor={Colors.pink} key={1} />,
      );
    }

    if (remote) {
      travelSettings.push(
        <TravelSetting badgeText="Remote" badgeColor={Colors.teal} key={2} />,
      );
    }
