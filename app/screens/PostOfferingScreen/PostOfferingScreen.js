...
import type { InputValidity } from "types/InputValidity";
...
class PostOfferingScreen extends AgentScreen {
  ...
  onHoursChanged = (newHours: number) => {
    this.setState({ currentHours: newHours });
  };

  onMinutesChanged = (newMinutes: number) => {
    this.setState({ currentMinutes: newMinutes });
  };
  ...
  getInputValidity = (): InputValidity => {
    const { currentTitle, currentDescription, currentHours, currentMinutes } = this.state;
  ...
  postOfferings = async () => {
    try {
      ...
      this.setState({ isLoading: true, isFinishPressed: true });
      await Promise.all(
      ...
      this.clearOfferingsList();
      AlertUtils.showSnackBar("Posted offerings successfully", Colors.primaryLight);
      this.goToScreenWithService("OfferingListScreen");
    } catch (error) {
      console.log(error);
      AlertUtils.showSnackBar("Something went wrong");
    } finally {
      this.setState({ isLoading: false, isFinishPressed: false });
    }
  };
  ...
  onFinishPressed = () => {
    ...
    if (!offerings.length) {
      this.goToScreenWithService("OfferingListScreen", { shouldUpdateAgent: true });
    } else {
      this.postOfferings();
    }
  };
  ...
  render() {
    ...
    return (
      ...
      >
        {isTimeModalVisible && (
          <View>
            <Modal isVisible={isTimeModalVisible} onBackdropPress={this.toggleTimeModal}>
              <Card>
                <View>
                  <TimeInput
          ...
          <Card style={styles.offeringCard}>
            ...
            >
              ...
              <TimeInputPrompt
                ...
                toggleTimeModal={this.toggleTimeModal}
              />
              <PriceInput currentPrice={currentPrice} onPriceChanged={this.onPriceChanged} />
              <View style={styles.stepContainer}>
