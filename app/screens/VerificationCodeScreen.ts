...
class VerificationCodeScreen extends Component<Props, State> {
  ...
  onVerificationCodeInputChanged = verificationCode => {
  ...
  onVerifyCodePressed = async () => {
    ...
    try {
      this.setState({ isLoading: true, isVerifyPressed: true });
      if (!verificationCode) {
        AlertUtils.showSnackBar("That code does not look right");
        return;
      }
      await this.authenticationManager.verifyCode(email, verificationCode);
      this.setState({ isLoading: false, isVerifyPressed: false });
      this.toggleShuffle();
      navigation.navigate({ routeName: "SuccessfulSignupScreen" });
    } catch (error) {
      this.setState({ isLoading: false, isVerifyPressed: false });
      if (error.name === AuthStatus.COGNITO.EXPIRED_CODE_ERROR) {
        AlertUtils.showSnackBar(`Verification code expired, new code sent to ${email}`);
