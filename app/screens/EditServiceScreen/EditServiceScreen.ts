...
class EditServiceScreen extends AgentScreen {
  ...
  getSectionListData = () => {
    ...
    if (service) {
      sections = editServiceSections.map(({ field, title, fields }) => {
        let data = [{ fieldId: field, text: `${service[field]}` }];
        ...
        if (fields) {
          data = fields.map(({ fieldTitle, fieldId }) => {
            ...
            return { fieldId, text: `${fieldTitle}: ${fieldValue}` };
          });
        }
      ...
      this.setState(
        ...
        async () => {
          ...
          if (serviceValidity.isValid) {
            await this.serviceManager.updateService(fieldId, newSwitchValue, service._id);
  ...
  onSavePressed = async () => {
    ...
    try {
      ...
      const serviceValidity = ValidationUtils.getServiceValidity(service);
      if (serviceValidity.isValid) {
        await this.serviceManager.updateService(currentEditFieldId, currentEditValue, _id);
        AlertUtils.showSnackBar("Updated service!", Colors.primaryLight);
      } else {
        AlertUtils.showSnackBar(serviceValidity.reason);
      }
  ...
  getPlaceholder = () => {
    ...
    const foundSection = editServiceSections.find(section => currentEditFieldId === section.field);
