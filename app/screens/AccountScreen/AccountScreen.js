...
import type { ModalType, PublicInfoSectionType, StatsType } from "./AccountScreenTypes";
...
class AccountScreen extends AgentScreen {
  ...
  async componentDidMount(): Promise<any> & any {
    ...
    this.setState(prevState => ({ isLoading: false, originalAgent: prevState.agent }));
  ...
  handleNavigationAction = () => {
    ...
    const shouldSetProfilePicture = navigation.getParam("shouldSetProfilePicture");
  ...
  toggleModalMenuVisibility = (onToggleModalMenuVisibility = () => {}) => {
    this.setState(
      prevState => ({ isModalMenuOpen: !prevState.isModalMenuOpen }),
      () => {
      ...
      this.setState(
        ...
        () => {
          this.toggleInputLocationModalVisibility(async () => {
            ...
            AlertUtils.showSnackBar("Updated location successfully", Colors.primaryLight);
          });
        },
      );
    } catch (error) {
  ...
  onListItemChanged = (input, index) => {
    ...
    this.updateAgentListStates(newCertifications, newEducation, newLanguages, newSkills);
  };

  updateAgentListStates = (newCertifications, newEducation, newLanguages, newSkills) => {
    this.setState(prevState => ({
      agent: {
        ...prevState.agent,
  ...
  dismissChanges = () => {
    ...
    const { certifications, education, languages, skills, company } = originalAgent;
    this.setState(prevState => ({
      agent: {
        ...prevState.agent,
        company,
  ...
  onSaveButtonPressed = async (editModalId: ModalType) => {
    try {
      ...
      this.closeModal();
      AlertUtils.showSnackBar("Updated account successfully!", Colors.primaryDark);
    } catch (error) {
      console.log(error);
      AlertUtils.showSnackBar("Something went wrong :(");
    }
  ...
  saveProfilePicture = async picture => {
    ...
    try {
      ...
      await this.agentManager.updateAgent("profilePictureUrl", newProfilePictureUrl);
      this.setState(prevState => ({
        agent: {
          ...prevState.agent,
      ...
      const serviceCarousel = _.isEmpty(services) ? (
        ...
        <View style={styles.serviceCarouselContainer}>
          <Carousel
            ...
            slideStyle={{ marginTop: 16, marginBottom: 16 }}
        ...
        <>
          ...
          <View style={styles.servicesContainer}>
            <View style={styles.servicesHeadingContainer}>
              <Heading title="Active Services" onAddPressed={this.onAddServicePressed} />
            </View>
            {serviceCarousel}
          </View>
          ...
          <Modal
            ...
            isVisible={editModalId !== null && editModalId !== ProfileListSetting.LOCATION}
