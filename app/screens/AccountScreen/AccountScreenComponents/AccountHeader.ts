...
import type { StatsType } from "../AccountScreenTypes";
...
const AccountHeader = (props: Props) => {
  const { profilePictureUrl, stats, firstName, lastName, onProfilePicturePressed } = props;
        ...
        <View style={styles.headerInfoContainer}>
          ...
          <View style={styles.headerStatsContainer}>
            {stats.map((stat, index) => (
              <StatBox key={index} first={index === 0} count={stat.count} title={stat.title} />
            ))}
          </View>
        </View>
      </View>
