...
class SignupCredentialsScreen extends Component<Props, State> {
  ...
  getInputValidity = () => {
    ...
    if (!FormatUtils.isValidEmailFormat(email)) {
      return { isValid: false, reason: "That email doesn't look right" };
    }
    if (password.length < 8) {
      return { isValid: false, reason: "That password is too easy to guess" };
    }
    if (confirmPassword.length < 1) {
      return { isValid: false, reason: "Please enter all fields" };
    }
    if (!(confirmPassword === password)) {
      return { isValid: false, reason: "Enter the same password" };
    }
    return { isValid: true };
  }
  ...
  onCredentialsEntered = async () => {
    ...
    try {
      ...
      this.setState({ isLoading: true, isSignupPressed: true });
      await this.authenticationManager.signup(firstName, lastName, email, password);
      this.toggleShuffle();
      navigation.navigate({
      ...
      this.setState({ isLoading: false, isSignupPressed: false });
    }
  };
